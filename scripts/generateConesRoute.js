...
function buildStraight(x, y, direction) {
    let additions = [[0,0], [0, 0.25], [0,0.5], [0,0.75], [0,1]]
    additions.push(...additions.map(item=>{return [item[0]+1, item[1]]}));
    if (direction == "N" || direction == "S") {
        cones.push(...additions.map(item=>{return {x:x+item[0], y:y+item[1] }}));
        //     [
        ...
        cones.push(...additions.map(item=>{return {x:x+item[1], y:y+item[0] }}));
...
map.tiles.forEach((row, y) => {
    row.forEach((item, x) => {
        ...
        if (item.startsWith('straight')) {
            buildStraight(x, y, item.split('/')[1], item)
        }
